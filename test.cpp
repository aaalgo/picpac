...
int main (int argc, char *argv[]) {
    ...
    map<unsigned, int> cnt_id;
    {
        ...
        for (unsigned i = 0; i < MAX_SEG_RECORDS * 3; ++i) {
            Record r;
            r.pack(rand() % C, fs::path("picpac.h"), lexical_cast<string>(i));
            r.meta->id = i;
            cnt[r.meta->label] += 5;
            cnt_id[i] += 5;
            out.append(r);
    ...
    for (unsigned F = 0; F < 5; ++F) {
        for (int train = 0; train < 2; ++train) {
            BasicStreamer::KFoldConfig conf(5, F, bool(train));
            conf.loop = false;
            BasicStreamer streamer(path, conf);
            for (;;) {
                try {
                    Record r;
                    streamer.read_next(&r);
                    cnt[r.meta->label] -= 1;
                    cnt_id[r.meta->id] -= 1;
                }
                catch (EoS) {
    ...
    for (int stra = 0; stra < 2; ++stra) {
        BasicStreamer::KFoldConfig conf(5, 3, true);
